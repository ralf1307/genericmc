...
public:
    ...
    void createMainToolbar(QMainWindow *MainWindow)
    {
        ...
        actionAddInstance->setIcon(GMC->getThemedIcon("new"));
        ...
        actionViewInstanceFolder->setIcon(GMC->getThemedIcon("viewfolder"));
        ...
        actionViewCentralModsFolder->setIcon(GMC->getThemedIcon("centralmods"));
        ...
        foldersMenuButton->setIcon(GMC->getThemedIcon("viewfolder"));
        ...
        actionSettings->setIcon(GMC->getThemedIcon("settings"));
        ...
        if (!BuildConfig.BUG_TRACKER_URL.isEmpty()) {
            ...
            actionReportBug->setIcon(GMC->getThemedIcon("bug"));
            actionReportBug.setTextId(QT_TRANSLATE_NOOP("MainWindow", "Report a Bug"));
            actionReportBug.setTooltipId(QT_TRANSLATE_NOOP("MainWindow", "Open the bug tracker to report a bug with GenericMC."));
        ...
        if (!BuildConfig.DISCORD_URL.isEmpty()) {
            ...
            actionDISCORD->setIcon(GMC->getThemedIcon("discord"));
            actionDISCORD.setTextId(QT_TRANSLATE_NOOP("MainWindow", "Discord"));
            actionDISCORD.setTooltipId(QT_TRANSLATE_NOOP("MainWindow", "Open GenericMC discord voice chat."));
        ...
        if (!BuildConfig.SUBREDDIT_URL.isEmpty()) {
            ...
            actionREDDIT->setIcon(GMC->getThemedIcon("reddit-alien"));
            actionREDDIT.setTextId(QT_TRANSLATE_NOOP("MainWindow", "Reddit"));
            actionREDDIT.setTooltipId(QT_TRANSLATE_NOOP("MainWindow", "Open GenericMC subreddit."));
        ...
        actionAbout->setIcon(GMC->getThemedIcon("about"));
        actionAbout->setMenuRole(QAction::AboutRole);
        actionAbout.setTextId(QT_TRANSLATE_NOOP("MainWindow", "About GenericMC"));
        actionAbout.setTooltipId(QT_TRANSLATE_NOOP("MainWindow", "View information about GenericMC."));
        ...
        helpMenuButton.setTooltipId(QT_TRANSLATE_NOOP("MainWindow", "Get help with GenericMC or Minecraft."));
        helpMenuButton->setMenu(helpMenu);
        helpMenuButton->setPopupMode(QToolButton::InstantPopup);
        helpMenuButton->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        helpMenuButton->setIcon(GMC->getThemedIcon("help"));
        ...
        if(BuildConfig.UPDATER_ENABLED)
        {
            ...
            actionCheckUpdate->setIcon(GMC->getThemedIcon("checkupdate"));
            actionCheckUpdate.setTextId(QT_TRANSLATE_NOOP("MainWindow", "Update"));
            actionCheckUpdate.setTooltipId(QT_TRANSLATE_NOOP("MainWindow", "Check for new updates for GenericMC."));
        ...
        actionPatreon->setIcon(GMC->getThemedIcon("patreon"));
        actionPatreon.setTextId(QT_TRANSLATE_NOOP("MainWindow", "Support GenericMC"));
        actionPatreon.setTooltipId(QT_TRANSLATE_NOOP("MainWindow", "Open the GenericMC Patreon page."));
        ...
        actionCAT->setIcon(GMC->getThemedIcon("cat"));
        ...
        actionManageAccounts->setIcon(GMC->getThemedIcon("accounts"));
    ...
    void createNewsToolbar(QMainWindow *MainWindow)
    {
        ...
        actionMoreNews->setIcon(GMC->getThemedIcon("news"));
        actionMoreNews.setTextId(QT_TRANSLATE_NOOP("MainWindow", "More news..."));
        actionMoreNews.setTooltipId(QT_TRANSLATE_NOOP("MainWindow", "Open the GenericMC development blog to read more news about GenericMC."));
    ...
    void createInstanceToolbar(QMainWindow *MainWindow)
    {
        ...
        changeIconButton->setIcon(GMC->getThemedIcon("news"));
        ...
        actionCopyInstance->setIcon(GMC->getThemedIcon("copy"));
    ...
    void setupUi(QMainWindow *MainWindow)
    {
        ...
        MainWindow->setWindowIcon(GMC->getThemedIcon("logo"));
        MainWindow->setWindowTitle("GenericMC 5");
#ifndef QT_NO_ACCESSIBILITY
        MainWindow->setAccessibleName("GenericMC");
#endif
    ...
    {
        QString winTitle = tr("GenericMC 5 - Version %1").arg(BuildConfig.printableVersionString());
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new MainWindow::Ui)
{
    ...
    // Create the instance list widget
    {
        ...
        proxymodel->setSourceModel(GMC->instances().get());
        proxymodel->sort(0);
        connect(proxymodel, &InstanceProxyModel::dataChanged, this, &MainWindow::instanceDataChanged);

        view->setModel(proxymodel);
        view->setSourceOfGroupCollapseStatus([](const QString & groupName)->bool {
            return GMC->instances()->isGroupCollapsed(groupName);
        });
        connect(view, &GroupView::groupStateChanged, GMC->instances().get(), &InstanceList::on_GroupStateChanged);
    ...
    connect(GMC->icons().get(), &IconList::iconUpdated, this, &MainWindow::iconUpdated);

    // model reset -> selection is invalid. All the instance pointers are wrong.
    connect(GMC->instances().get(), &InstanceList::dataIsInvalid, this, &MainWindow::selectionBad);

    // handle newly added instances
    connect(GMC->instances().get(), &InstanceList::instanceSelectRequest, this, &MainWindow::instanceSelectRequest);

    // When the global settings page closes, we want to know about it and update our state
    connect(GMC, &GenericMC::globalSettingsClosed, this, &MainWindow::globalSettingsClosed);
    ...
    accountMenuButton->setIcon(GMC->getThemedIcon("noaccount"));
    ...
    /*if(BuildConfig.UPDATER_ENABLED)
    {
        ...
        auto updater = GMC->updateChecker();
        ...
        if (GMC->settings()->get("AutoUpdate").toBool() && updatesAllowed)
        {
    ...
    setSelectedInstanceById(GMC->settings()->get("SelectedInstance").toString());
...
{
    std::shared_ptr<AccountList> accounts = GMC->accounts();
...
void MainWindow::showInstanceContextMenu(const QPoint &pos)
{
    ...
    else
    {
        ...
        QAction *actionVoid = new QAction("GenericMC", this);
        actionVoid->setEnabled(false);
    ...
    for (auto profiler : GMC->profilers().values())
    {
        ...
        else
        {
            connect(profilerAction, &QAction::triggered, [this, profiler]()
                    {
                        GMC->launch(m_selectedInstance, true, profiler.get());
                    });
            connect(profilerOfflineAction, &QAction::triggered, [this, profiler]()
                    {
                        GMC->launch(m_selectedInstance, false, profiler.get());
                    });
        }
    }
...
void MainWindow::repopulateAccountsMenu()
{
    ...
    action->setCheckable(true);
    action->setIcon(GMC->getThemedIcon("noaccount"));
    action->setData("");
...
void MainWindow::changeActiveAccount()
{
    ...
    GMC->accounts()->setActiveAccount(id);
...
void MainWindow::activeAccountChanged()
{
    ...
    MinecraftAccountPtr account = GMC->accounts()->activeAccount();
...
void MainWindow::notificationsChanged()
{
    ...
    QList<int> shownNotifications = stringToIntList(GMC->settings()->get("ShownNotifications").toString());
    ...
    GMC->settings()->set("ShownNotifications", intListToString(shownNotifications));
}
...
void MainWindow::downloadUpdates(GoUpdate::Status status)
{
    ...
    status.rootPath = GMC->root();

    auto dlPath = FS::PathCombine(GMC->root(), "update", "XXXXXX");
    ...
    if (updateDlg.execWithTask(&updateTask))
    {
         ...
         */
        GMC->updateIsRunning(true);
        UpdateController update(this, GMC->root(), updateTask.updateFilesDir(), updateTask.operations());
        update.installUpdates();
        GMC->updateIsRunning(false);
    }
    else
    {
...
{
    unique_qobject_ptr<Task> task(GMC->instances()->wrapInstanceTask(rawTask));
...
void MainWindow::on_actionCopyInstance_triggered()
{
    ...
    unique_qobject_ptr<Task> task(GMC->instances()->wrapInstanceTask(copyTask));
    runModalTask(task.get());
}
...
void MainWindow::finalizeInstance(InstancePtr inst)
{
    ...
    if (GMC->accounts()->anyAccountIsValid())
    {
        ProgressDialog loadDialog(this);
    ...
    {
        CustomMessageBox::selectable(this, tr("Error"), tr("GenericMC cannot download Minecraft or update instances unless you have at least "
...
void MainWindow::addInstance(QString url)
{
    ...
    {
        groupName = GMC->settings()->get("LastUsedGroupForNewInstance").toString();
    }
    ...
    GMC->settings()->set("LastUsedGroupForNewInstance", newInstDlg.instGroup());
...
void MainWindow::on_actionChangeInstIcon_triggered()
{
    ...
    if (dlg.result() == QDialog::Accepted)
    {
        ...
        auto icon = GMC->icons()->getIcon(dlg.selectedIconKey);
...
void MainWindow::iconUpdated(QString icon)
{
    ...
    {
        auto icon = GMC->icons()->getIcon(m_currentInstIcon);
        ui->actionChangeInstIcon->setIcon(icon);
        ui->changeIconButton->setIcon(icon);
    }
}
...
void MainWindow::setSelectedInstanceById(const QString &id)
{
    if (id.isNull())
        return;
    const QModelIndex index = GMC->instances()->getInstanceIndexById(id);
    if (index.isValid())
    {
...
void MainWindow::on_actionChangeInstGroup_triggered()
{
    ...
    QString name(GMC->instances()->getInstanceGroup(instId));
    auto groups = GMC->instances()->getGroups();
    groups.insert(0, "");
    ...
    if (ok)
    {
        GMC->instances()->setInstanceGroup(instId, name);
    }
}
...
void MainWindow::deleteGroup()
{
    ...
    if(!groupName.isEmpty())
    {
        ...
        {
            GMC->instances()->deleteGroup(groupName);
        }
    }
}
...
{
    QString str = GMC->settings()->get("InstanceDir").toString();
    DesktopServices::openDirectory(str);
}

void MainWindow::refreshInstances()
{
    GMC->instances()->loadList();
}

void MainWindow::on_actionViewCentralModsFolder_triggered()
{
    DesktopServices::openDirectory(GMC->settings()->get("CentralModsDir").toString(), true);
}
...
void MainWindow::checkForUpdates()
{
    ...
    {
        auto updater = GMC->updateChecker();
        updater->checkForUpdate(GMC->settings()->get("UpdateChannel").toString(), true);
    }
    else
    {
...
{
    GMC->ShowGlobalSettings(this, "global-settings");
}
...
{
    GMC->showInstanceWindow(m_selectedInstance, "settings");
}

void MainWindow::on_actionEditInstNotes_triggered()
{
    GMC->showInstanceWindow(m_selectedInstance, "notes");
}

void MainWindow::on_actionWorlds_triggered()
{
    GMC->showInstanceWindow(m_selectedInstance, "worlds");
}

void MainWindow::on_actionEditInstance_triggered()
{
    GMC->showInstanceWindow(m_selectedInstance);
}

void MainWindow::on_actionScreenshots_triggered()
{
    GMC->showInstanceWindow(m_selectedInstance, "screenshots");
}

void MainWindow::on_actionManageAccounts_triggered()
{
    GMC->ShowGlobalSettings(this, "accounts");
}
...
{
    DesktopServices::openUrl(QUrl("https://www.patreon.com/GenericMC"));
}

void MainWindow::on_actionMoreNews_triggered()
{
    DesktopServices::openUrl(QUrl("https://GenericMC.org/posts.html"));
}
...
void MainWindow::on_actionDeleteInstance_triggered()
{
    ...
    {
        GMC->instances()->deleteInstance(id);
    }
}
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    GMC->settings()->set("MainWindowState", saveState().toBase64());
    GMC->settings()->set("MainWindowGeometry", saveGeometry().toBase64());
    event->accept();
    emit isClosing();
}
...
void MainWindow::instanceActivated(QModelIndex index)
{
    ...
    InstancePtr inst = GMC->instances()->getInstanceById(id);
    if (!inst)
        return;
...
void MainWindow::on_actionLaunchInstance_triggered()
{
    ...
    {
        GMC->kill(m_selectedInstance);
    }
    else
    {
        GMC->launch(m_selectedInstance);
    }
}
...
void MainWindow::instanceChanged(const QModelIndex &current, const QModelIndex &previous)
{
    if (!current.isValid())
    {
        GMC->settings()->set("SelectedInstance", QString());
        selectionBad();
        return;
    }
    QString id = current.data(InstanceList::InstanceIDRole).toString();
    m_selectedInstance = GMC->instances()->getInstanceById(id);
    if (m_selectedInstance)
    {
        ...
        GMC->settings()->set("SelectedInstance", m_selectedInstance->id());
    }
    else
    {
...
{
    QString instanceFolder = GMC->settings()->get("InstanceDir").toString();
    if (FS::checkProblemticPathJava(QDir(instanceFolder)))
    {
        ...
        warning.setInformativeText(tr("You have now two options: <br/>"
                                      ...
                                      " - move this installation of GenericMC5 to a different folder"));
        warning.setDefaultButton(QMessageBox::Ok);
        warning.exec();
    }
    auto tempFolderText = tr("This is a problem: <br/>"
                             " - GenericMC will likely be deleted without warning by the operating system <br/>"
                             " - close GenericMC now and extract it to a real location, not a temporary folder");
    QString pathfoldername = QDir(instanceFolder).absolutePath();
    if (pathfoldername.contains("Rar$", Qt::CaseInsensitive))
    {
        QMessageBox warning(this);
        warning.setText(tr("Your instance folder contains \'Rar$\' - that means you haven't extracted the GenericMC zip!"));
...
{
    int timeplayed = GMC->instances()->getTotalPlayTime();
