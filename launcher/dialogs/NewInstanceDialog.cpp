    ...
    : QDialog(parent), ui(new Ui::NewInstanceDialog)
{
    ...
    setWindowIcon(GMC->getThemedIcon("new"));

    InstIconKey = "default";
    ui->iconButton->setIcon(GMC->icons()->getIcon(InstIconKey));

    auto groups = GMC->instances()->getGroups().toSet();
    ...
    restoreGeometry(QByteArray::fromBase64(GMC->settings()->get("NewInstanceGeometry").toByteArray()));
}

void NewInstanceDialog::reject()
{
    GMC->settings()->set("NewInstanceGeometry", saveGeometry().toBase64());
    QDialog::reject();
}
...
void NewInstanceDialog::setSuggestedPack(const QString& name, InstanceTask* task)
{
    ...
    if(!task)
    {
        ui->iconButton->setIcon(GMC->icons()->getIcon("default"));
        importIcon = false;
    }
...
{
    auto icon = GMC->icons()->getIcon(key);
    importIcon = false;
...
{
    if(importIcon) {
        GMC->icons()->installIcon(importIconPath, importIconName);
