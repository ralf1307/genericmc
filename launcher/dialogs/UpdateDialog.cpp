...
{
    ui->setupUi(this);
    auto channel = GMC->settings()->get("UpdateChannel").toString();
    if(hasUpdate)
    {
    ...
    loadChangelog();
    restoreGeometry(QByteArray::fromBase64(GMC->settings()->get("UpdateDialogGeometry").toByteArray()));
}
...
{
    GMC->settings()->set("UpdateDialogGeometry", saveGeometry().toBase64());
