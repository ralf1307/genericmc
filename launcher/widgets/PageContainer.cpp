...
void PageContainer::createUI()
{
    ...
    const int leftMargin = GMC->style()->pixelMetric(QStyle::PM_LayoutLeftMargin);
    ...
    const int rightMargin = GMC->style()->pixelMetric(QStyle::PM_LayoutRightMargin);
...
void PageContainer::showPage(int row)
{
    ...
    else
    {
        ...
        m_iconHeader->setIcon(GMC->getThemedIcon("bug"));
    }
}
