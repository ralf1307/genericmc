    ...
    : QMainWindow(parent), m_instance(instance)
{
    ...
    auto icon = GMC->icons()->getIcon(m_instance->iconKey());
    ...
    {
        auto base64State = GMC->settings()->get("ConsoleWindowState").toByteArray();
        restoreState(QByteArray::fromBase64(base64State));
        auto base64Geometry = GMC->settings()->get("ConsoleWindowGeometry").toByteArray();
...
{
    GMC->launch(m_instance, false, nullptr);
}
...
void InstanceWindow::closeEvent(QCloseEvent *event)
{
    ...
    GMC->settings()->set("ConsoleWindowState", saveState().toBase64());
    GMC->settings()->set("ConsoleWindowGeometry", saveGeometry().toBase64());
    emit isClosing();
    event->accept();
}
...
void InstanceWindow::on_btnKillMinecraft_clicked()
{
    ...
    {
        GMC->launch(m_instance, true, nullptr);
    }
}
