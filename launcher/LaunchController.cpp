    ...
    if (accounts->count() <= 0)
    {
        ...
        auto reply = CustomMessageBox::selectable(
            ...
            tr("In order to play Minecraft, you must have at least one Mojang or Minecraft "
               "account logged in to GenericMC."
        ...
        if (reply == QMessageBox::Yes)
        {
            ...
            GMC->ShowGlobalSettings(m_parentWidget, "accounts");
        }
    }
...
void LaunchController::launchInstance()
{
    ...
    {
        GMC->showInstanceWindow(m_instance);
    }
    ...
    if(m_session->wants_online) {
        ...
        m_launcher->prependStep(new TextPrint(m_launcher.get(), resolved_servers, MessageLevel::GenericMC));
    } else {
        online_mode = "offline";
    }
    ...
    // Prepend Version
    m_launcher->prependStep(new TextPrint(m_launcher.get(), "GenericMC version: " + BuildConfig.printableVersionString() + "\n\n", MessageLevel::GenericMC));
    m_launcher->start();
}
...
void LaunchController::onFailed(QString reason)
{
    ...
    {
        GMC->showInstanceWindow(m_instance, "console");
    }
    emitFailed(reason);
}
