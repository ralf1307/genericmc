...
IconType GMCIcon::type() const
{
    return m_current_type;
}

QString GMCIcon::name() const
{
    if (m_name.size())
        return m_name;
    return m_key;
...
bool GMCIcon::has(IconType _type) const
{
    return m_images[_type].present();
}

QIcon GMCIcon::icon() const
{
...
void GMCIcon::remove(IconType rm_type)
{
...
void GMCIcon::replace(IconType new_type, QIcon icon, QString path)
{
...
void GMCIcon::replace(IconType new_type, const QString& key)
{
    if (new_type > m_current_type || m_current_type == IconType::ToBeDeleted)
    {
        m_current_type = new_type;
...
QString GMCIcon::getFilePath() const
{
    if(m_current_type == IconType::ToBeDeleted){
        return QString();
    }
